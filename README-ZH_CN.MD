...
1. 创建PM2配置文件`ecosystem.config.js`
...
module.exports = {
  apps: [{
    ...
    args: '8888 n',//n:启动服务时不需要打开浏览器，server.js中，当参数为n时openBrowser=false
    cwd: __dirname,//配置文件所在的目录作为Node.js应用运行时的工作目录，配置文件放在dist下
    instances: 1,
    autorestart: true,
    watch: false,
